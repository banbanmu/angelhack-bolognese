...
public class Common{
  ...
  public enum STAT_CODE_TYPE {
    ...
    /** 4: Invalid parameters of the video profile of the mixed video. See <a href="hhttps://docs.agora.io/en/faq/recording_video_profile?_ga=2.4402419.1965239098.1566358977-1833718170.1540974595">Video Profile Table</a> to set the `mixResolution` parameter. */
  ...
  /** The reasons why the recording server leaves channel. */
  public enum LEAVE_PATH_CODE {
    ...
    /** 4: The recording server calls the {@link io.agora.recording.RecordingSDK.leaveChannel leaveChannel} method to leave the channel. */
  ...
  public enum CONNECTION_STATE_TYPE {
     ...
     * <ul>
     *  <li>When the app calls the {@link io.agora.recording.RecordingSDK#createChannel createChannel} method, the SDK starts to establish a connection to the specified channel, triggers the {@link io.agora.recording.RecordingEventHandler#onConnectionStateChanged onConnectionStateChanged} callback, and switches to the {@link io.agora.recording.common.Common.CONNECTION_STATE_TYPE#CONNECTION_STATE_CONNECTING CONNECTION_STATE_CONNECTING} state.
     *  <li>When a user successfully joins a channel, the SDK triggers the {@link io.agora.recording.RecordingEventHandler#onConnectionStateChanged onConnectionStateChanged} callback and switches to the {@link io.agora.recording.common.Common.CONNECTION_STATE_TYPE#CONNECTION_STATE_CONNECTED CONNECTION_STATE_CONNECTED} state.
     ...
     * <li>The {@link io.agora.recording.RecordingEventHandler#onConnectionStateChanged onConnectionStateChanged} callback, and switches to the {@link io.agora.recording.common.Common.CONNECTION_STATE_TYPE#CONNECTION_STATE_RECONNECTING CONNECTION_STATE_RECONNECTING} state.
     * </ul>
     */
    CONNECTION_STATE_CONNECTED(3),
    /** 4: The SDK keeps rejoining the channel after being disconnected from a joined channel because of network issues.
     * <ul>
     *  <li>If the SDK cannot join the channel within 10 seconds after being disconnected from Agora's edge server, the SDK triggers the {@link io.agora.recording.RecordingEventHandler#onConnectionLost onConnectionLost} callback, stays in the {@link io.agora.recording.common.Common.CONNECTION_STATE_TYPE#CONNECTION_STATE_RECONNECTING CONNECTION_STATE_RECONNECTING} state, and keeps rejoining the channel.
     *  <li>If the SDK fails to rejoin the channel 20 minutes after being disconnected from Agora's edge server, the SDK triggers the {@link io.agora.recording.RecordingEventHandler#onConnectionStateChanged onConnectionStateChanged} callback, switches to the {@link io.agora.recording.common.Common.CONNECTION_STATE_TYPE#CONNECTION_STATE_FAILED CONNECTION_STATE_FAILED} state, and keeps rejoining the channel.
     * </ul>
     */
  ...
  public enum VIDEO_FORMAT_TYPE {
    ...
    /** 6: Video frame in H.264 or H.265 format. */
  ...
  public class AudioFrame {
    ...
    /** If type is AUDIO_FRAME_RAW_PCM, then PCM field points to PCM format audio data, AAC should not be used. See the structure of {@link io.agora.recording.common.Common#AudioPcmFrame AudioPcmFrame}. */
    public AudioPcmFrame pcm;
    /** If type is AUDIO_FRAME_AAC, then AAC field points to AAC format audio data, PCM should not be used. See the structure of {@link io.agora.recording.common.Common#AudioAacFrame AudioAacFrame}. */
  ...
  public class VideoFrame {
    /** If VideoFrame is in YUV format, then YUV points to video data in YUV format, H.264 and JPG fields should not be used. See {@link io.agora.recording.common.Common#VideoYuvFrame VideoYuvFrame}. */
    public VideoYuvFrame yuv;
    /** If VideoFrame is in H.264 format, then H.264 points to video data in H.264 format, YUV and JPG fields should not be used. See {@link io.agora.recording.common.Common#VideoH264Frame VideoH264Frame}. */
    public VideoH264Frame h264;
    /** If VideoFrame is in H.265 format, then H.265 points to video data in H.265 format, YUV and JPG fields should not be used. See {@link io.agora.recording.common.Common#VideoH265Frame VideoH265Frame}. */
    public VideoH265Frame h265;
    /** If VideoFrame is in JPG format, then JPG points to video data in JPG format, YUV and H.264 fields should not be used. See {@link io.agora.recording.common.Common#VideoJpgFrame VideoJpgFrame}. */
  ...
  public class VideoMixingLayout
  {
     ...
     *
     * @note If you set the {@link io.agora.recording.common.RecordingConfig#defaultVideoBgPath defaultVideoBgPath} parameter in recordingconfig when calling the {@link io.agora.recording.RecordingSDK#createChannel createChannel} method, the `backgroundColor` parameter does not take effect.
     */
    ...
    /** The users (communication mode) or hosts (live broadcast mode) list of VideoMixingLayout. Each user (communication mode) or host (live broadcast mode) in the channel has a region to display the video on the screen. See {@link io.agora.recording.common.Common#VideoMixingLayout#Region Region} to set parameters. */
    ...
    /** Configurates text watermarks. Pointer to an array of {@link io.agora.recording.common.Common#LiteraWatermarkConfig LiteraWatermarkConfig}.
     *
     * @note You can add up to ten text watermarks.
     */
    public LiteraWatermarkConfig[] literalWms;
    /** Configurates a timestamp watermark. Pointer to {@link io.agora.recording.common.Common#TimestampWatermarkConfig TimestampWatermarkConfig}.
     *
     * @note You can only add one timestamp watermark.
     */
    public TimestampWatermarkConfig[] timestampWms;
    /** Configurates image watermarks. Pointer to an array of {@link io.agora.recording.common.Common#ImageWatermarkConfig ImageWatermarkConfig}.
     *
